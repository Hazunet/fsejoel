...
def check_coin_collect(p, coin_count):
...
def draw_enemies(cam):
    for enemy in enemies:
        draw.rect(screen, red, (enemy[0] - cam[0], enemy[1] - cam[1], enemy[2], enemy[3]))

def check_enemy_collision(p, vel, state, coin_count):
...
def update_physics(p, vel, cam, state):
    ...
    if state["invincibility"] > 0:
        state["invincibility"] -= 1

def handle_input(p, vel, state):
    keys = key.get_pressed()

    if state["invincibility"] < 100: 
        if (keys[K_SPACE] or keys[K_UP]) and not state["is_jumping"]:
            vel[1] = -25
...
def render(p, cam, coin_count, state):
    cam[0] += ((p[X] - w // 2) - cam[0]) * 0.08
    cam[1] += ((p[Y] - h // 2) - cam[1]) * 0.08

    screen.blit(bg_img, (-cam[0], -2720 - cam[1]))

    draw_coins(cam, coin_count)
    draw_dropped_coins(cam)
    draw_enemies(cam)

    if state["invincibility"] % 4 < 2:
...
def play_level():
    ...
    vel   = [0, 0]
    cam   = [0, 0]
    state = {"is_jumping": False, "invincibility": 0}
    coin_count = 0
    ...
    while True:
        ...
        update_coins()
        update_enemies(cam)
        update_dropped_coins(cam)
        
        coin_count = check_coin_collect(p, coin_count)
        coin_count = collect_dropped_coins(p, coin_count)
        coin_count = check_enemy_collision(p, vel, state, coin_count)
        
        render(p, cam, coin_count, state)
