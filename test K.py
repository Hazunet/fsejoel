...
spike_colour = (83,83,83)   # colour of the spike tips in the background

def check_spikes(p, cam, vel, invincibility, coin_count, lives, dropped_coins):
    """Colour-detect spikes directly under Sonic's feet (3 sample points)."""
    if invincibility > 0:
        return invincibility, coin_count, lives

    foot_y = int(p[Y] + p[H] - cam[1])     # screen-space y just below feet

    # Sample left edge, centre, right edge of hitbox
    sample_xs = [
    ...
    if hit:
        # Same knockback as enemy: bounce up and sideways
        vel[1] = -12
...
def play_level():
    ...
    while True:
        ...
        # Spike check must happen AFTER render so screen pixels are up to date
