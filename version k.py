...
def draw_hud(cam, coin_count, lives, coins, elapsed_time):
    for coin in coins:
        if coin[5] == False:
...
def draw_enemies(cam, enemies, fishes, bombers, projectiles, win_box):
...
def render(p, cam, coin_count, lives, invincibility, coins, enemies, fishes, bombers, projectiles, dropped_coins, is_rolling, elapsed_time, win_box):
    cam[0] += ((p[X] - w // 2) - cam[0]) * 0.08
    cam[1] += ((p[Y] - h // 2) - cam[1]) * 0.08

    screen.blit(bg_img, (-cam[0], -2720 - cam[1]))

    draw_hud(cam, coin_count, lives, coins, elapsed_time)
    draw_dropped_coins(cam, dropped_coins)
    draw_enemies(cam, enemies, fishes, bombers, projectiles, win_box)
...
def play_level():
    ...
    while True:
        ...
        render(p, cam, coin_count, lives, invincibility, coins, enemies, fishes, bombers, projectiles, dropped_coins, is_rolling, elapsed_time, win_box)
