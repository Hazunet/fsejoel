...
HOVER_OFFSET=50
SCAN_DEPTH=300
WALL_SCAN_DIST=60   # how far ahead of player we scan for walls
...
def check_wall(player):
    """Scan horizontally from player sides, return 'left', 'right', or None."""
    # scan from middle height of player on screen
    scan_y=height//2
    ...
    for scan_x in range(right_edge, min(right_edge+WALL_SCAN_DIST, width)):
        try:
            ...
            break
        if colour_close(col,WALL_COLOUR,TOLERANCE):
            return 'right'
...
def drawScene(player, offsetX, offsetY):
    screen.blit(sonicBg,(offsetX, offsetY))
    draw.rect(screen,blue,[vel[screenX]-player[rW]//2, height//2-player[rH]//2, player[rW], player[rH]])
...
def level1():
    ...
    while running:
        ...
        offsetY=height//2-player[Y]-2720

        # background drawn first so pixel scans read correct colours
        drawScene(player,offsetX,offsetY)

        # check ground and wall before moving
        check(player)
        wall=check_wall(player)
        move(player,wall)

        # redraw player on top after position update
        draw.rect(screen,blue,[vel[screenX]-player[rW]//2, height//2-player[rH]//2, player[rW], player[rH]])
        ...
        try:
            ...
            col=(0,0,0)
        screen.blit(gfont.render(f"feet pixel: {col}",True,black),(10,10))
        screen.blit(gfont.render(f"wall: {wall}",True,black),(10,34))
        screen.blit(gfont.render(f"world X:{int(player[X])}  Y:{int(player[Y])}",True,black),(10,58))
        screen.blit(gfont.render("arrows move   Space jump   Esc back",True,black),(10,82))
