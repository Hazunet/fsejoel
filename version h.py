    ...
    for b in bombers:
        if b[7] > 0:
            ...
            if b[8] == True:
                ...
                if abs(p[X] - b[0]) < 250 and p[Y] > b[1]:
                    b[8] = True
                    b[7] = 30
                    proj_vx = -5 if p[X] < b[0] else 5
...
def update_projectiles(projectiles):
    for pr in projectiles:
        pr[0] += pr[4]
        pr[1] += pr[5]
    return [pr for pr in projectiles if -500 < pr[0] < 40000 and pr[1] < h + 500]
...
def check_enemy_collision(p, vel, invincibility, coin_count, lives, enemies, fishes, bombers, projectiles, dropped_coins):
    if invincibility <= 0:
        player_rect = Rect(p[X], p[Y], p[W], p[H])
        hit = False
        enemy_x = 0
        ...
        for b in bombers:
            if player_rect.colliderect(Rect(b[0], b[1], b[2], b[3])):
                hit = True
                enemy_x = b[0]

        for pr in projectiles:
            if player_rect.colliderect(Rect(pr[0], pr[1], pr[2], pr[3])):
                hit = True
                enemy_x = pr[0]
...
def render(p, cam, coin_count, lives, invincibility, coins, enemies, fishes, bombers, projectiles, dropped_coins):
...
def play_level():
    ...
    while True:
        ...
        invincibility, coin_count, lives = check_enemy_collision(p, vel, invincibility, coin_count, lives, enemies, fishes, bombers, projectiles, dropped_coins)
        
        render(p, cam, coin_count, lives, invincibility, coins, enemies, fishes, bombers, projectiles, dropped_coins)
